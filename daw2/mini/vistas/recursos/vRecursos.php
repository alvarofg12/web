...
//echo $local;
foreach($tipos as $nombreTipo => $registros) {
	$url= '?'.http_build_query( array('a'=>'recursos.verTipo', 'tipo'=>$nombreTipo, 'p'=>$pagina));
    echo '<a href="'.$url.'"><h1>'.$nombreTipo.'</h1></a>';
...
foreach($registros as $indice => $registro) {
  ...
  vista::generarPieza( 'ficha_recurso', array( 'articulo'=>$modelo, 'pagina'=>$pagina));
